...
class Movement(object):
    ...
    def step(self, movement_callback_obj: MovementCallback):
        ...
        if left_color == color.BLACK or right_color == color.BLACK:
            movement_callback_obj.on_black(self, left_color == color.BLACK, right_color == color.BLACK)

        if left_color == color.BLUE or right_color == color.BLUE:
            movement_callback_obj.on_blue(self, left_color == color.BLUE, right_color == color.BLUE)

        if left_color == color.GREEN or right_color == color.GREEN:
            movement_callback_obj.on_green(self, left_color == color.GREEN, right_color == color.GREEN)

        if left_color == color.YELLOW or right_color == color.YELLOW:
            movement_callback_obj.on_yellow(self, left_color == color.YELLOW, right_color == color.YELLOW)

        if left_color == color.RED or right_color == color.RED:
            movement_callback_obj.on_red(self, left_color == color.RED, right_color == color.RED)

        if left_color == color.WHITE or right_color == color.WHITE:
            movement_callback_obj.on_white(self, left_color == color.WHITE, right_color == color.WHITE)
